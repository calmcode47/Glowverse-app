...
export async function requestPasswordReset(email: string): Promise<{ success: boolean; message: string }> {
  const res = await client.post<{ success: boolean; message: string }>("/api/v1/auth/forgot-password", { email });
  return res.data;
}

export async function verifyResetToken(token: string): Promise<{ success: boolean; data?: { email: string } }> {
  const res = await client.get<{ success: boolean; data?: { email: string } }>(`/api/v1/auth/reset-password/${token}`);
  return res.data;
}

export async function resetPassword(token: string, password: string): Promise<{ success: boolean; message: string }> {
  const res = await client.post<{ success: boolean; message: string }>("/api/v1/auth/reset-password", { token, password });
  return res.data;
}
